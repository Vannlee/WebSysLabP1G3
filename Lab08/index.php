        ...
        if ($result->num_rows > 0){
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<img class="img-thumbnail" src="' . $row["image_path"] . '" 
                        ...
                        data-location="' . htmlspecialchars($row["loc_addr"]) . '" 
                        data-booking_slots="' . htmlspecialchars($row["morning_slot"]) . ', ' . htmlspecialchars($row["afternoon_slot"]) . '"
                        data-contact="' . htmlspecialchars($row["loc_contact"]) . '">';
                echo '</figure>';
